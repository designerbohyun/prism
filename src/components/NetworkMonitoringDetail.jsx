...
function NetworkMonitoringDetail({ isDarkMode, cctvInfo, onBack }) {
  ...
  const generateSampleData = () => {
    ...
    for (let i = 23; i >= 0; i--) {
      ...
      for (let j = 0; j < 6; j++) {
        ...
        const detailStatus = Math.random() > 0.05 ? "SUCCESS" : Math.random() > 0.5 ? "FAIL" : "TIMEOUT";
        
        tableData.push({
          id: `${i}-${j}`,
          timestamp: detailTime.toISOString(),
          status: detailStatus,
          rttMax: detailStatus === "SUCCESS" ? Math.floor(Math.random() * 20) + 30 : 0,
          rttAvg: detailStatus === "SUCCESS" ? Math.floor(Math.random() * 15) + 20 : 0,
          rttMin: detailStatus === "SUCCESS" ? Math.floor(Math.random() * 10) + 10 : 0,
          packetLoss: detailStatus === "SUCCESS" ? Math.random() * 2 : detailStatus === "TIMEOUT" ? 100 : Math.random() * 50,
        });
      }
    }
  ...
  const toggleColumn = (columnKey) => {
    ...
    localStorage.setItem("networkMonitoringColumns", JSON.stringify(newVisibleColumns));
  };
        ...
        <div className={`p-3 rounded-lg border shadow-lg ${
          ...
          {payload.map((entry, index) => (
            <p key={index} style={{ color: entry.color }} className="text-sm">
              {entry.name}: {entry.value}{entry.dataKey.includes("RTT") ? "ms" : entry.dataKey === "PACKET_LOSS_RATE" ? "%" : ""}
            </p>
          ))}
        </div>
      );
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={320}>
              <LineChart data={data}>
                ...
                <Legend />
                <Line yAxisId="ms" type="monotone" dataKey="RTT_MAX" stroke="#ef4444" strokeWidth={2} name="RTT MAX" dot={{ r: 2 }} />
                <Line yAxisId="ms" type="monotone" dataKey="RTT_AVG" stroke="#f59e0b" strokeWidth={2} name="RTT AVG" dot={{ r: 2 }} />
                <Line yAxisId="ms" type="monotone" dataKey="RTT_MIN" stroke="#10b981" strokeWidth={2} name="RTT MIN" dot={{ r: 2 }} />
                <Line yAxisId="percent" type="monotone" dataKey="PACKET_LOSS_RATE" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 5" name="PACKET LOSS" dot={{ r: 2 }} />
                <Line yAxisId="ms" type="stepAfter" dataKey="STATUS" stroke="#22c55e" strokeWidth={3} name="STATUS" dot={false} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
              ...
              <div className={`absolute right-0 mt-2 w-48 rounded-lg border shadow-lg z-10 ${
                ...
                <div className="p-2">
                  {Object.entries({
                    ...
                    <label key={key} className="flex items-center p-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                      <input
                        type="checkbox"
